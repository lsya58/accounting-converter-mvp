...
| Yayoi | Yayoi Accounting AE 19 | 弥生インポート形式 direct export | Export evidence | 25 observed | no header observed | CP932 observed | 2000 observed | OBSERVED | private raw export observation | Medium for observed 2000 single-record subset | 複数の独立単一仕訳、借方補助科目ありrecord、借方部門ありrecordで再観測。正式YayoiInputAdapterや万能仕様へ昇格しない。 |
...
- 弥生会計AE19 direct exportで、25項目、CP932、CRLF、BOMなし、識別フラグ2000、ヘッダーなし、貸借一致候補をraw bytes/parserで観測した。複数の独立単一仕訳、借方補助科目ありrecord、借方部門ありrecordでも同じ構造が再観測された。
